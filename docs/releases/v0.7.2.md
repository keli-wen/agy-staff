...
The diagnosis and the fix design were reviewed independently by an agy review job, which confirmed the cause and identified the `taskkill /T` path; a second review of the final diff found no blocker and produced the two follow-ups above.

## Upgrading

Windows users of 0.7.1 should upgrade. Claude Code and Codex install a copy, so pull the new version in (see [upgrading](../REFERENCE.md#upgrading)) and restart the harness. A job started under 0.7.1 and still running during the upgrade keeps working and can be canceled; its worker runs the old code until it finishes.
