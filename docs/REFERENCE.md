...
|---|---|---|
| `implement` | dirty workspaces are treated as bounded prompt context. When the tree is dirty, the companion injects a capped pre-run `git status --porcelain` summary into the implement prompt so agy treats those paths as user-owned context. Afterward it reports whether the workspace is clean, changed, or still dirty | warns that agy's edits cannot be reviewed or rolled back via git, then proceeds |
...
- **Dirty workspace on implement** — the companion treats dirty workspaces as bounded prompt context. It adds a capped pre-run status summary to the prompt, tells agy those paths are user-owned context, and instructs agy to use `git status --porcelain` and `git diff` as ground truth when path ownership is unclear. If existing changes look unrelated, agy should ask before overwriting, cleaning, stashing, resetting, deleting, committing, pushing, or opening a PR with them.
