...
> [!IMPORTANT]
> **There is no mandatory setup step.** `staffer`, `researcher`, `reviewer` and `implementer` run **unrestricted** by default: agy gathers evidence and edits files on its own, guarded by the prompt templates (no costly side effects) plus `implementer` workspace decisions and prompt-selected `diff` / `commit` / `pr` delivery contracts.
...
| Implement and open a draft PR | `/agy:implementer fix the flaky retry test and open a draft PR` |
