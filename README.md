...
> [!IMPORTANT]
> **There is no mandatory setup step.** `staffer`, `researcher`, `reviewer` and `implementer` run **unrestricted** by default: agy gathers evidence and edits files on its own, guarded by prompt templates. `implementer` now treats dirty workspaces as prompt context instead of a hard refusal, and agy may commit/push/open a PR only when the task explicitly asks for that delivery.
